...
private:
    WiFiClient* wifiClient;  // ✅ MUDADO: WiFiClient normal (sem TLS)
    ...
    int bdSeq;  // ✅ NOVO: Birth/Death sequence number (SparkplugB)
...
public:
    ...
    // ✅ NOVO: SparkplugB - Publicações do Node
    bool publicarNBIRTH();
    bool publicarNDATA();
    bool publicarDCMD(int idRemota, const String& comando, int valor = 0);
...
private:
    // ✅ NOVO: SparkplugB - Construtores de Payloads
    const char* construirPayloadNDEATH();
    String construirPayloadNBIRTH();
    String construirPayloadNDATA();
    String construirPayloadDCMD(const String& comando, int valor);
};
